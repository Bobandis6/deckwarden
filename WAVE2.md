...
- [x] W4 — Choose-commander flow (+ `/commanders?q=`) (2026-09-19: feat `077401a`; D3 + REC-2 shipped — `display.leaderBrowse` on both adapters, leader-zone primary Browse link + ghost "Search by name", `leader-pick-intent.ts` (sessionStorage, 30-min TTL, null server snapshot), `HubBuildCta` + `LeaderPickBanner` islands (hub CTA server HTML verified byte-identical to prod, 1019/990 bytes), editor `?leader=` apply through `handleAdd` with param strip + intent clear, `?q=` on `/commanders` AND `/leaders` (lockstep) over `ci_name_trgm` (0.09 ms Bitmap Index Scan). Decisions recorded in `W4-session-prompt.md`'s ship note: banner sits above the q-form/chips row; q resets page; a full zone with a matching intent announces on the search pane's live line via a new `SearchPaneHandle.announce` and the intent is left to TTL (cleared on apply/Cancel only, contract-literal). Round trip proven E2E on dev both paths; QA guest deck deleted after (census restored to 25).)
- [x] W5 — Card-page printings gallery + printings API (+ default-printing fix) (2026-09-20: feat `34a5d41`; D4 + REC-1 shipped — provider around the server identity block with `PrintingsHero`/`PrintingsTable` leaves, `?printing=` via `printing-param.ts` (replaceState + null server snapshot, resolved against known rows so crafted ids change nothing), ≤100 slim rows inline + `GET /api/cards/[id]/printings` (250 cap, `{total, truncated}`, 1h/24h edge cache, 0.77 ms through `cp_by_identity`), decode-then-crossfade hero keeping the old image as underlay, phone Drawer via `ui/drawer.tsx` as-is, `CardNamePreview` widened to `{name, image, caption?}`. REC-1 ordering landed with a `default_rank_rule` stats marker; read-only replay: 2,826 winners re-point at the next nightly, Sol Ring SLD 2783 → MSC 212 ($1.57). Decisions recorded in `W5-session-prompt.md`'s ship note: identity/legality/combos stay server-rendered; slim row = id/set/collector/rarity/year/isDefault/hasBack/usd/usdFoil + imageOverride only when non-null; drawer = image + set line + prices, the W7 buy slot is an empty structural footer (no dead control); tail lives in provider state, button reads "Loading…", fetch errors keep it retryable; hero caption (Flip/Reset) renders on phones too — the drawer has no pin action, so Flip must stay reachable; page + API now both exclude `isRemoved` (removed printings previously rendered). LATER row added for the >250 deep-link + filter-before-Show-all edges.)
- [x] W6 — Editor printings ("Use this printing in deck") (2026-09-20: feat `2c8a755`; D5 shipped — Printings `Collapsible` in the card pane fed by W5's API on FIRST open only (per-card cache per pane instance, count in the trigger once known, N=1 honest), row clicks preview in the pane image without editing, "Use this printing in deck" is a real edit through **(new)** pure `setPrinting()` → `applyEdit` → autosave, D5 toast with an Undo that re-applies the previous choice as its own edit, in-memory `cards` map + `preview` take the chosen image so list/grid/leader follow at once, ambient art refetches via `useLeaderArt`'s printingId key. Decisions recorded in `W6-session-prompt.md`'s ship note: `setPrinting` addresses the CARD (every entry — `EditorCard.image` is per card, per-zone choices could never render), applying a row always pins it explicitly (even the default row; clearing back to follow-the-default has no D5 control), not-in-deck cards get the list viewer-only (the TagEditor idiom), OP rows have no price cells and spacer thumbs, est.-price footnote inside the collapsible for priced games. Round trip + Undo + commander crossfade proven on dev (QA guest deck deleted, census 25 re-proven); `editor-state.test.ts` +4, `deck-editor.test.tsx` +2, create-count pins untouched.)
- [x] W7 — Buy links (plain; affiliate env stays empty on Hobby) (2026-09-20: feat `e1b94c1`; D6 shipped — `capabilities.buy` on the adapter seam (MTG → TCGplayer; One Piece declares nothing, so no buy surface exists there, no apology copy), **(new)** `src/lib/buy/links.ts` + `src/lib/games/mtg/buy.ts` fully unit-tested, share-row "Buy this deck" menu (Whole deck / Without basic lands / Only cards I'm missing — the last only when `owned` is present, per-copy math matching "You own N/M"), editor More → "Buy this deck…" dialog (no `owned` there: the editor's lazily-grown set can't tell "no collection" from "owns nothing") + pane-footer Buy ↗, card-page hero link built SERVER-side and passed as `{href, vendor, rel}` data (route stays ●, no "sponsored" literal in the client bundle) + the phone drawer's [ Buy ↗ ] in W5's reserved footer. Mass Entry URL shape live-verified before coding (`?productline=Magic&c=1%20Sol%20Ring%7C%7C33%20Mountain` pre-fills + auto-opens Item Options); **the contract's name-rule sketch was overruled by the live form**: full adventure names are REJECTED, so splits/aftermath/rooms keep "A // B" while adventures/omens (type_line probe) and transform/MDFC use the front face — parse acceptance on real decks: Krenko whole deck 69/70 (miss: Goblin Chirurgeon, TCGplayer's FEM "(A)/(B)" variant catalog), Niv-Mizzet 80 lines → 100 cart items incl. "Expansion // Explosion", MDFC front-face lines 2/2; known misses recorded: Kamigawa flips (TCGplayer names them "Front (Back)" — undetectable without `layout`), FEM multi-art commons. ~6k cliff measured on the FINAL url → copy-list + bare form + toast (share view mounts its own `Toaster`; clipboard-refused = error toast, no stranded tab). Affiliate dark: `next.config.ts` pins a build-time `""` so an UNSET env var can't survive as a runtime lookup — `rel="sponsored"` + all disclosure lines fold out of `.next/static` (grep-proven); env-set dev run proved the wrap (double-encoding), sponsored rel, and disclosures in menu/site footer//legal. `/legal` "Buying cards" section; `editor-header.test.tsx:129` pin updated (+ OP-negative case); `seo-smoke:374` + all smokes green; 865 tests / 113 files.)
- [x] W8a — Precons data (migration, MTGJSON ingest, `kind` guards) (2026-09-20: feat `1b1a7c4` + de-flake `89035ab`; migration `0014_reflective_impossible_man` matched the contract's expected SQL statement-for-statement (no reconcile needed; applied once, `kind='precon'` count 0 before ingest) — `decks.kind` + both CHECKs + `decks_recent_public` rebuilt with `kind='user'` (Bitmap Index Scan proven under `enable_seqscan=off`; the seq-scan preference at 188 rows is planner economics) + `precon_products` (code = MTGJSON **fileName**, the only unique key in DeckList). Ingest: **175 of 191** "Commander Deck" entries live in prod — the 16 skips are all Collector's Editions with a same-set regular sibling (identical name×count lists, different foil printings; verified byte-equal on 40K), zero other skips, zero fetch failures, **printings_unresolved 0**, slug collisions 0; rerun `unchanged: 175` (source_hash over the MAPPED subset — MTGJSON re-stamps `meta.date` daily so raw hashes and the `.sha256` sidecars can never match twice). Live-file drift captured in the mapper header + real trimmed fixture (TimeyWimey): multi-face cards are ONE row (`side:"a"`, full "A // B" name) and the same oracle id recurs across rows as DIFFERENT printings of a basic (Plains #196×2 + #197×1 → merged, higher-count printing represents the entry — order-independence forced by the hash test). Weekly gate lives IN THE SCRIPT: nightly = new codes only (a skipped new set retries the night after Scryfall has it), Sunday UTC or `PRECONS_FULL=true` (wired to workflow_dispatch) = full re-fetch; nightly step landed after Scryfall, dispatch-proven green same day (backup line lists `precon_products`; restore drill re-run green). Descriptions on `decks.description` (feeds og:description; `precon_products.blurb` reserved NULL for W8b). Guards: home rail + `loadHubDecks` + `combos-smoke` `kind='user'` (Atraxa live: 1 user deck shown, 2 precons excluded), purge dry-run 0 candidates with 157 release-dated precons in range, `deckMetaJson` exposes `kind`, access tests pin the unowned shape, live refusals proven on dev AND prod (PATCH/PUT/DELETE/forged-token 403, claim/like 401 signed out, GET 200, sitemap 175 `/d/p_` URLs). db 263.2 → 266.4 MB (+3.2 < 6). CI flake: the W7 Buy-menu test lost its 50 ms race on the Node-24 runner — `pollFor` (fake-time slices + one real `setImmediate` turn per round) landed as `89035ab`; the `-t` isolation coupling predates W8a (reproduced at `bcde7ea`) and is a LATER row. 884 tests / 114 files.)
- [x] W8b — Precons surfaces (`/precons`, share-page meta, Start from this precon) (2026-09-21: feat `e2df10c`; D7 + REC-4 shipped over **181** products (the Sunday full sweep had landed 6 new FDC/FRC 2026-10-02 entries since W8a's 175 — the weekly gate working, not drift). `/precons` = one static ISR page (`revalidate 86400`, ○ in the build), all 181 tiles in the HTML grouped by release year (15 groups 2011–2026), client filter island (`precons-index-view.tsx`): search over a pre-normalized name+commanders+set haystack, exact-identity color pills (the /commanders semantics; 1 colorless precon makes C real), year + sort as styled NATIVE selects (`ui/select.tsx` has zero users — no popup machinery to stall in a hidden pane), the island sorts by releaseDate itself (never trusts input order); **filter state ephemeral by decision** (no searchParams per D7, no hash either — pinned by the never-touches-the-URL test). `GET /api/precons/[slug]` public `s-maxage=86400` returns meta + `fetchDeckCardsWire` rows, precon-only by construction (query starts from precon_products); **lookup-key decision: the SLUG, never the MTGJSON fileName code** (uppercase fails `PRECON_SLUG_RE` → 404, pinned in `precon-info.test.ts`; REC-4's `from=` carries the slug too). Share page joins by deck.id (`loadPreconByDeckId` — the join is the test, never the `p_` prefix): badge + meta line "Preconstructed deck · Commander 2016 (C16) · Released Nov 2016" + "Official product list · data via MTGJSON" byline + **Start from this precon** before Fork; **summary decision**: colors/commander/release stay in the W8a `decks.description` sentence, curve stays in the Analytics section, and the render-computed half is est.-price-today + five priciest off the wire's live `cheapestUsd` (SUM skips null-priced cards → honest ≈); `precon_products.blurb` **stays NULL**. No "Updated" anywhere on product surfaces (tiles carry `dateLabel` "Released Nov 2016"; `DeckTile` grew additive `badge` prop + `dateLabel`/`priceLabel` fields — home rail proven clean after). Hub shelf `loadHubPrecons` (public + kind='precon' + `leader_ids @>`, release-desc via updated_at) ABOVE the community shelf + conditional MTGJSON credit; Atraxa live: 2 tiles (C16+CM2) over 1 community deck; Krenko: no shelf. Editor: `draftFromSlug` latched in the chooser (`from` supersedes `leader`), seeds the WHOLE list state-only with the precon's own printingIds (never `applyImport`), name state-only with the meta baseline moved, **`ensureDeck` now sends `metaRef.name` in the create POST** (the route always accepted `name`) so the acceptance is exact: proven live on dev — zero POST while seeded (100/100 Warden-green, ambient art already on the precon's Atraxa printing), first edit = one POST 201 (row named "Breed Lethality") + one PUT with all 83 printingIds byte-equal to `deck_cards`, no PATCH; Suggestions populate immediately; unknown/game-mismatched `from=` seeds nothing + visible error toast (live-caught at 200 ms — a 5 s toast outlives pane round-trips only if you poll in the same batch). Nav: Precons after Cards per the D1 sketch (both `site-header.test.tsx` arrays), `/precons` in the sitemap. 901 tests / 116 files; `smoke:hubs` + `smoke:seo` green; QA deck deleted, census 25 restored.)
- [x] W9a — Autofill engine + API (2026-09-21: feat `02ee64b`; the five steps shipped in order — engine refactor proven invisible (`gatherSignals(snapshot, opts)` owns candidate assembly, the tournament-signals read hoisted to `recommendForSnapshot`/the route so the union is read ONCE; 70/70 recommend tests + `smoke:recommend` byte-identical before/after), whitelisted `CandidateFilter.scope` + `loadEntryFacts` + `loadFillerRows`, `wireSelect`/`toWire` → `src/lib/cards/wire.ts` (`loadCardWires(ids, formatId)`; `fetchDeckCardsWire` untouched), `RecommendMeta.autofill` implemented only in MTG (37 + Σ curve buckets = 99 pinned; `lockShare` references `TOURNAMENT_STAPLE_SHARE`; `[6,8,16,22,26,28]` is `rankedByColorCount` — the lands-by-color ladder, NOT a curve template), pure two-phase planner (`buildShell`/`finishShell` + mulberry32/Efraimidis–Spirakis in `rng.ts`) with locked/combo/sampled/filler tiers, nearest-bucket borrowing (b−1 first) and a pinned notes vocabulary, and `POST /api/decks/autofill` (force-dynamic no-store, `deckAutofill` 20/min + 200/h, writes nothing; OP → 400). Decisions in `W9a-session-prompt.md`'s ship note: the isBase split is the SQL scope (`eq`/`ne` on the adapter's declared base scope), the lock rule reads the CANDIDATE's lists ≥ lockMinLists, leaders are excluded from the planner's curve state, fillers resolve in a second pure phase (pip split needs cost texts the wire read supplies), notes name group labels. Acceptance all green on dev (`pnpm smoke:autofill`: Atraxa 99/validate-clean/evidence-everywhere, Kinnan locked tier, Talrand zero topdeck rows, partners 98, Kozilek Wastes, seeds identical/32-different, $1 budget honest, 429 at 21/min) and prod spot-checked ($5 budget → 99 picks at $99.01, seed-identical twice). **14 statements** per call measured with DB_LOG=1 (2 rate + facts + 2 pools + combos + 2 tournament pools + 3 signal reads + fillers + wire + legality). 924 tests / 117 files.)
- [x] W9b — Review sheet + editor doors (2026-09-21: feat `e551042`; D8's sheet + the two EDITOR doors shipped behind `adapter.recommend?.autofill` — **(new)** `autofill-sheet.tsx` (Dialog `wide` from md / bottom Drawer on phones, ModalFinalFocus via the Modal shell), one POST per open + per control change with the cleanup abort (StrictMode's dev double-fire lands as one 200), skeleton rows, budget chips All/≤$5/≤$1 → `budgetUsd`, "Keep my N cards" (hidden at N=0), Reroll disabled in flight, **no seed ever sent** (the response's seed held in state, rendered nowhere — shareable seed stays W9c-adjacent), notes verbatim, source-count line from `sourceMeta` labels, evidence sentence + REQUIRED source credit link per row with "+N more" expand, native checkboxes (indeterminate group header), live Apply count + client-computed est over CHECKED picks, `issues` block above the footer, 429 sentence + Retry disabled for the Retry-After window. **Pinned decisions** (module doc + tests): sheet state dies with the dialog (modal ⇒ no stale-shell case; reopen refetches), one row per pick entry ("Wastes × 11"), keep-unchecked Apply REPLACES the non-leader list (leaders + picks), keep-checked stacks on the whole current list. Apply goes through **(new)** `applyListSwap(entries, cards, title)` — captures the previous list, toasts Undo that restores it via markDirty, `mergeEntries()` (new pure helper) folds duplicate (zone, card) rows — and **Import now rides the same path, gaining Undo** ("Import applied"). Doors: EmptyState "Autofill a starter shell" only once a leader is set (beside "Add cards"), More → "Autofill…" after Import (`editor-header.test.tsx` pin updated + OP-negative case; `EditorDialog` grew "autofill"). Proven live on dev: Atraxa draft → door → 99 picks (88 EDHREC + 11 land-template, "37 + Σ buckets = 99" visible in the group counts), ≤$1 roll max $0.99 still 99, Apply = toast "Added 99 cards · Undo" + Warden-green 100/100 + exactly ONE extra PUT (network-verified), full-deck reopen = verbatim FULL note + "Add 0 cards" disabled, keep-unchecked full-curve re-roll; RTL: draft apply mints ONE deck + one PUT, Undo restores + autosaves, reroll/cancel never create or dirty, OP editor shows neither door. No server-side changes. 940 tests / 118 files; `smoke:recommend` + `smoke:autofill` green; QA deck deleted, census 25 restored.)
- [x] W9c — Doors: Surprise me · hub · combos (2026-09-22: feat `2c6c4a6`; the remaining three D8 doors + two GET routes. **(new)** `GET /api/leaders/random?game=` — force-dynamic no-store, own `leadersRandom` bucket (30/min per IP: uncacheable by design), the staples' banned/not_legal `NOT EXISTS` (loadLeaderIndex keeps banned for reference; a surprise must be playable as rolled), Magic uniform inside `SURPRISE_POOL = 100` most-played / OP uniform over all legal slugged leaders, and it returns the **full CardWire** so the draft seeder spends no resolve unit — a surprise draft fires **zero POSTs** until a real edit (network-verified). **(new)** `GET /api/cards/[id]/combos?fit=` — public `s-maxage=3600, stale-while-revalidate=86400` over loadCombosForCard (pieces now carry `externalKey`, additive on ComboPieceRef), no rate bucket (art-route stance), unknown ids = honest empty 200. **Doors**: (2) Surprise me on the /decks/new Magic card + home Magic card (`?surprise=1`, latched + adjust-during-render for the picker's own button, stripped via replaceState with `autofill` so a reload never re-rolls; seed-only — no auto-sheet, D8's roll-to-shell chain → LATER; OP's roll works by URL, doorless → LATER); (3) hub CTA row "Start with a starter shell" (`&autofill=1`) behind a new additive `autofill` prop on HubBuildCta gated `adapter.recommend?.autofill` — pinned anchors byte-identical, /l/ renders none; the editor's `?autofill=1` open is a **render-time latch** (set-state-in-effect is an eslint error) that waits for the seed to SETTLE, not succeed — a failed seed or crafted bare link opens the sheet to its honest "set a commander first" sentence, POST-free (pinned); (4) Combo Radar "With your commander" — fetched once per (anchor, fit) leader change (useLeaderArt discipline), renders in a seeded DRAFT with no deck row, in-deck ✓s client-side over inDeckQty, top 5 of the API's 10 with the honest "Showing the N most-played of M" line; **Add N pieces** = ONE resolve by externalKey (pass 0, id-guarded per piece) + quiet panel adds + ONE toast; **Build around** = the same adds quietly, then openAutofill — the pieces ride as keeps and the plan builds around them (proven live: Kiki-Jiki draft, Add 2 pieces = 1 resolve + "Added 2 combo pieces", Build around = "Keep my 3 cards" + "Add 96 cards" = 100). No "Combo piece" tag (sheet renders no tags — LATER). Proven live on dev end-to-end incl. OP surprise (Rayleigh, no sheet anywhere) and the hub round trip (StrictMode's aborted autofill POST folds to one landed 200). 950 tests / 119 files (+combo-radar-panel.test.tsx, +4 editor RTL, +hub CTA case); `smoke:autofill` grew the two GET sections; hubs/recommend smokes green; QA deck deleted, census 25.)
- [x] W10 — Tournaments v1 (2026-09-22: feat `92b574a`; D9 shipped — **(new)** `/tournaments` force-dynamic index (ƒ in the build; `?game=` like /cards, `GameSwitch` custom hrefs, 50 newest events via `tournaments_game_date` with the placement-1 winner's leader(s) hub-linked + ColorChips; `?leader=<slug>` swaps in that leader's full run, `ALL_FINISHES_CAP` 100, unknown slug = real 404) and **(new)** `/tournaments/[id]` ISR trio (● in the build, `revalidate 86400` + `generateStaticParams []`, id regex + existence gate in the segment LAYOUT above the loading boundary via React-cached `./event.ts` — junk ids answer honest HTTP 404s, curl-proven on dev and prod; `robots: { index: false }` per the D9 v1 decision) rendering the standings table in placement order with player names as published, `LeaderNames` (new shared component: hub link when slugged + chips; partner pairs dot-joined), records (`title` says draws vs ties per game), external `Decklist ↗` + the event's own external source link on the meta line. Queries: `loadTopFinishes` gained `tournaments.id` + a limit param (hub callers unchanged); **(new)** `loadRecentTournaments` + `loadTournamentEvent` return leader `{name, slug, colorsMask}` via the same card_identities subquery precedent — both pages read tournaments + tournament_standings only. Hub shelves (/c/ AND /l/): event names now link internally, the external event link **stays on every row** as the arrow anchor (sr-only "on {source}"; hubs-smoke's Topdeck/Limitless URL pins green untouched, + new additive W10 internal-link pins), "All {n} finishes →" under the list (`?game=optcg&leader=` on /l/ — the `&amp;` HTML escape is what the smoke greps). `BROWSE_LINKS` + both header-test arrays gained Tournaments last (D1 order); core sitemap lists the index's two game canonicals ONLY (seo-smoke pins: listed index, no event URLs, event noindex, junk 404). ISR proven on prod: `x-nextjs-prerender: 1`, second hit `x-vercel-cache: HIT`; index no-store/MISS. Decklist absence on Tymna's filtered run is honest data (0 of her 100 newest finishes carry URLs). 950 tests / 119 files; hubs/seo/optcg smokes green. **Wave 2 complete.**)
