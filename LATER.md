...
| Tag-editing UI in the editor (entries carry `tags text[]`; P1.3's tag grouping renders an honest "Untagged" bucket but nothing can set tags yet) | First editor session of M2. **P2.2 (2026-08-30) explicitly ruled itself out**: profiles/folders never opened the editor surface (`src/components/editor/*` untouched — folder assignment lives on /account). **P2.3 (2026-08-30) likewise ruled itself out**: likes/bookmarks/rail live on the share page, /account, and home — editor untouched again. **P2.4 (2026-08-30) same**: hubs are read-only card-data pages. Likely first firing: P2.7 (sample hand + deck notes). |
| Smarter mana-source detection (P1.5's `producedMask` counts conditional producers like Command Tower as all five colors; fetches/treasure-makers not counted) plus role tagging and curve targets | User feedback that the sources table misleads, or the next analytics-engine session. **P2.4 (2026-08-30) was the "first M2 analytics-adjacent session" and took the call: still deferred** — the hub's role template is editorial adapter constants and its curve is a fresh pure function over staples; neither touches analyze.ts's producedMask. Role *tagging* now has a natural home when tag editing lands (P2.7). |
| Share pages serve HTTP 200 with a denial shell for private decks — and, since P2.2, private folders (/f/[publicId]) — (the data itself is safe — never in the HTML; a real 403 status needs Next's authInterrupts/forbidden()) | P2.6 SEO pass, or crawlers start indexing denial pages. |
...
| "Decks with this commander" shelf on /c/[slug] hubs — the plumbing has existed since P1.1 (`decks.leader_ids` + the `decks_hub` GIN index) and P2.4 deliberately shipped hubs without it (v1a contract was staples/template/budget only). Cold-start rule applies: render the shelf only when ≥1 public deck actually has that leader. | P2.5 (hub v1b) session, or the first public decks with leaders exist — whichever comes first. |
