...
| Tag-editing UI in the editor (entries carry `tags text[]`; P1.3's tag grouping renders an honest "Untagged" bucket but nothing can set tags yet) | First editor session of M2 (P1.8 spent its room on the gate checklist; share pages already render the honest "Untagged" bucket, so nothing lies meanwhile). |
